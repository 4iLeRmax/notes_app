...
export default function LabelItem({ label, menuIsOpen }: LabelItemProps) {
  ...
  return (
    <>
      <div className="py-2 snap-start w-full">
        <Link
          href={`/labels/${label.id}`}
          className={cn(
            "w-full flex items-center gap-2 p-2 bg-primary rounded-3xl  text-txt-primary hover:text-custom-blue transition-colors",
            {
